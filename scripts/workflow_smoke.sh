...
SCENE_NAME="${SCENE_NAME:-venom_transform}"
TITLE="${TITLE:-flowbridge smoke test}"
COUNT="${COUNT:-5}"
...
submit_one() {
  ...
  code="$(curl -sS -o "$response" -w "%{http_code}" \
    ...
    --data-urlencode "title=$TITLE #$index" \
...
for i in $(seq 1 "$COUNT"); do
  submit_one "$i" || submit_failures=$((submit_failures + 1)) &
  active=$((active + 1))
  if (( active >= CONCURRENCY )); then
    wait -n || submit_failures=$((submit_failures + 1))
    active=$((active - 1))
  fi
done
wait || submit_failures=$((submit_failures + 1))
...
for poll in $(seq 1 "$MAX_POLLS"); do
  ...
  while IFS= read -r task_id; do
    ...
    code="$(curl -sS -o "$response" -w "%{http_code}" "$BASE_URL/api/public/task?task_id=$task_id")"
    if [[ "$code" != "200" ]]; then
